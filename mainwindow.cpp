...
void MainWindow::on_actionjson_triggered() {
    ...
    QString fileName = QFileDialog::getSaveFileName(
        ...
        tr("Save File"),
        "output_strategy.bin",
        tr("Binary Frames (*.bin);;Gzip 压缩的 JSON (*.json.gz);;JSON 文件 (*.json)"),
        &selectedFilter
        );
    ...
    // 工具：设置/替换后缀（支持多段）
    auto setExt = [&](const QString& wanted){ // wanted 例如 "json.gz"
        // 已知多段/单段后缀都剥掉，只留主干
        static const QStringList known = {"bin","json.gz","json"};
        for (const auto& k : known) {
            const QString dotk = "." + k;
            if (fileName.endsWith(dotk, Qt::CaseInsensitive)) {
                fileName.chop(dotk.size());
                break;
            }
        }
        fileName += "." + wanted;
    ...
    // 支持的后缀集合
    const QStringList allowed = {"json.gz","json","bin"};

    // 若用户已输入合法后缀 -> 尊重，不改
    if (!allowed.contains(ext)) {
        // 否则按选择的过滤器补后缀
        if (selectedFilter.contains("*.json.gz")) {
            setExt("json.gz");
        } else if (selectedFilter.contains("*.ndjson.gz")) {
            setExt("ndjson.gz");
        } else if (selectedFilter.contains("*.sqlite") || selectedFilter.contains("*.db")) {
            // 统一用 .sqlite（也可根据需要保留 .db）
            setExt("sqlite");
        } else if (selectedFilter.contains("*.ndjson")) {
            setExt("ndjson");
        } else if (selectedFilter.contains("*.json")) {
            setExt("json");
        } else if (selectedFilter.contains("*.bin")) {
            setExt("bin");
        }
        // 重新取一次后缀，便于后续日志/分支使用
        ext = QFileInfo(fileName).completeSuffix().toLower();
    }
